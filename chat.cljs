...
(defn get-system-prompt
  ...
  (p/create
   (fn [resolve reject]
     (go
       (try
         (if-let [cached-prompt
                  (get @prompt-cache session-id)]
...
(defn generate-response
  ...
  (p/create
   (fn [resolve reject]
     (go
       (try
         (let [system-prompt-data
               (<p! (get-system-prompt
                     session-id AGENTS_MD_PATH))
               system-prompt
               ...
               (take-last
                20 (into (vec (or (get-in @prompt-cache
                         ...
                         [(str (or (:name chat-doc)
                          ...
                          (str BOT_NAME ": "
                               response-text)]))]
           (swap! prompt-cache
                  assoc-in [session-id :chat-history]
                  updated-chat-history)
           (if opt-out
             (swap! prompt-cache
                    utils/dissoc-in [session-id :user]))
           (<p! (firestore/upsert-doc
                 "chats" session-id
                 {:chat-history updated-chat-history}))
  ...
  (try
    (let [gemini-key
          (<p! (.readFile
                ...
                GEMINI_KEYS_PATH))
          key
          (js->clj (.parse js/JSON gemini-key)
